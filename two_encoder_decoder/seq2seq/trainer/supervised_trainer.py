...
class SupervisedTrainer(object):
    ...
    def __init__(self, loss1=NLLLoss(), loss2=NLLLoss(), batch_size=64,
        ...
        self.loss1 = loss1
        self.loss2 = loss2
        self.evaluator = Evaluator(loss1=self.loss1, loss2=self.loss2, batch_size=batch_size)
        self.optimizer = None
    ...
    def get_model_out(self, input_variable1, input_variable2,  input_lengths1, input_lengths2, target_variable1,
                      target_variable2, model, teacher_forcing_ratio):
        d1, d2 = model(input_variable1, input_variable2, input_lengths1, input_lengths2, target_variable1,
                       target_variable2, teacher_forcing_ratio=teacher_forcing_ratio)
        return d1, d2
    ...
    def _train_epoches(self, data1, model, n_epochs, start_epoch, start_step,
        ...
        for epoch in range(start_epoch, n_epochs + 1):
            ...
            for batch1 in batch_generator1:
                ...
                d1, d2 = self.get_model_out(input_variables1, input_variables2, input_lengths1, input_lengths2,
                                            target_variables1, target_variables2, model, teacher_forcing_ratio)
                loss1 = self._train_batch(d1, target_variables1, model, self.loss1)
                d1, d2 = self.get_model_out(input_variables1, input_variables2, input_lengths1, input_lengths2,
                                            target_variables1, target_variables2, model, teacher_forcing_ratio)
                ...
                if step % self.print_every == 0 and step_elapsed > self.print_every:
                    ...
                    print_loss1_total = 0
                    log_msg1 = 'Progress: %d%%, Train %s: %.4f' % (
                ...
                if step % self.print_every == 0 and step_elapsed > self.print_every:
                    ...
                    print_loss2_total = 0
                    log_msg2 = 'Progress: %d%%, Train %s: %.4f' % (
                        step / total_steps * 100,
                        self.loss2.name,
                        print_loss2_avg)
                    log.info(log_msg2)
                ...
                if step % self.checkpoint_every == 0 or step == total_steps:
                    Checkpoint(model=model,
                               ...
                               output_vocab2=data1.fields[seq2seq.label_field_name2].vocab).save(self.expt_dir)
            if step_elapsed == 0:
                continue
            epoch_loss1_avg = epoch_loss1_total / min(steps_per_epoch, step - start_step)
            epoch_loss1_total = 0
            log_msg1 = "Finished epoch %d: Train %s: %.4f" % (epoch, self.loss1.name, epoch_loss1_avg)

            epoch_loss2_avg = epoch_loss2_total / min(steps_per_epoch, step - start_step)
            epoch_loss2_total = 0
            log_msg2 = "Finished epoch %d: Train %s: %.4f" % (epoch, self.loss2.name, epoch_loss2_avg)

            # 验证数据集
            if dev_data1 is not None:
                dev_loss1, dev_loss2, accuracy1, accuracy2 = self.evaluator.evaluate(model, dev_data1)

                self.optimizer.update(dev_loss1, epoch)
                self.optimizer.update(dev_loss2, epoch)
                log_msg1 += ", Dev %s: %.4f, Accuracy: %.4f" % (self.loss1.name, dev_loss1, accuracy1)
                model.train(mode=True)
                log_msg2 += ", Dev %s: %.4f, Accuracy: %.4f" % (self.loss2.name, dev_loss2, accuracy2)
                model.train(mode=True)
