...
class Checkpoint(object):
    ...
    MODEL_NAME = 'model.pt'
    INPUT_VOCAB_FILE1 = 'input_vocab1.pt'
    # INPUT_VOCAB_FILE2 = 'input_vocab2.pt'
    OUTPUT_VOCAB_FILE1 = 'output_vocab1.pt'
    OUTPUT_VOCAB_FILE2 = 'output_vocab2.pt'
    OUTPUT_VOCAB_FILE3 = 'output_vocab3.pt'

    def __init__(self, model, optimizer, epoch, step, input_vocab1, output_vocab1, output_vocab2, output_vocab3, path=None):
        self.model = model
        self.optimizer = optimizer
        self.input_vocab1 = input_vocab1
        # self.input_vocab2 = input_vocab2
        self.output_vocab1 = output_vocab1
        self.output_vocab2 = output_vocab2
        self.output_vocab3 = output_vocab3
        self.epoch = epoch
        self.step = step
        self._path = path
    ...
    def save(self, experiment_dir):
        ...
        with open(os.path.join(path, self.INPUT_VOCAB_FILE1), 'wb') as fout:
            dill.dump(self.input_vocab1, fout)
        with open(os.path.join(path, self.OUTPUT_VOCAB_FILE1), 'wb') as fout:
            dill.dump(self.output_vocab1, fout)
        # with open(os.path.join(path, self.INPUT_VOCAB_FILE2), 'wb') as fout:
        #     dill.dump(self.input_vocab2, fout)
        with open(os.path.join(path, self.OUTPUT_VOCAB_FILE2), 'wb') as fout:
            dill.dump(self.output_vocab2, fout)
        with open(os.path.join(path, self.OUTPUT_VOCAB_FILE3), 'wb') as fout:
            dill.dump(self.output_vocab3, fout)
    ...
    def load(cls, path):
        ...
        with open(os.path.join(path, cls.INPUT_VOCAB_FILE1), 'rb') as fin:
            input_vocab1 = dill.load(fin)
        with open(os.path.join(path, cls.OUTPUT_VOCAB_FILE1), 'rb') as fin:
            output_vocab1 = dill.load(fin)
        # with open(os.path.join(path, cls.INPUT_VOCAB_FILE2), 'rb') as fin:
        #     input_vocab2 = dill.load(fin)
        with open(os.path.join(path, cls.OUTPUT_VOCAB_FILE2), 'rb') as fin:
            output_vocab2 = dill.load(fin)
        with open(os.path.join(path, cls.OUTPUT_VOCAB_FILE3), 'rb') as fin:
            output_vocab3 = dill.load(fin)
        optimizer = resume_checkpoint['optimizer']
        return Checkpoint(model=model, input_vocab1=input_vocab1,
                          output_vocab1=output_vocab1, output_vocab2=output_vocab2,
                          output_vocab3=output_vocab3, optimizer=optimizer,
