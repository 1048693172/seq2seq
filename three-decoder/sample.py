...
if opt.load_checkpoint is not None:
    ...
    def len_filter(example):
        return len(example.bl) <= max_len and len(example.label) \
               <= max_len and len(example.label2) <= max_len and len(example.label3) <= max_len
    ...
    label3.build_vocab(train, max_size=5000)
    bl_vocab = bl.vocab
    label1_vocab = label.vocab
    label2_vocab = label2.vocab
    label3_vocab = label3.vocab
    ...
    weight3 = torch.ones(len(label3.vocab))
    pad3 = label3.vocab.stoi[label3.pad_token]
    loss3 = Perplexity(weight3, pad3)
    ...
    if not opt.resume:
        ...
        decoder3 = DecoderRNN(len(label3.vocab), max_len, hidden_size * 2 if bidirectional else hidden_size,
                              dropout_p=0.5, use_attention=True, bidirectional=bidirectional,
                              eos_id=label3.eos_id, sos_id=label3.sos_id)

        seq2seq = Seq2seq(encoder1, decoder1, decoder2, decoder3)
        if torch.cuda.is_available():
            seq2seq.cuda()
    ...
    # train
    t = SupervisedTrainer(loss1, loss2, loss3, batch_size=32,
                          checkpoint_every=5000,
                          print_every=2000, expt_dir=opt.expt_dir)
...
torch.save(bl_vocab, "bl.input_vocab")
torch.save(label1_vocab, "label1.input_vocab")
torch.save(label2_vocab, "label2.input_vocab")
torch.save(label3_vocab, "label3.input_vocab")
predictor = Predictor(seq2seq, bl_vocab, label1_vocab, label2_vocab, label3_vocab)
while True:
